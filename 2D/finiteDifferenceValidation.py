#!/usr/bin/env python3
"""
Created on Mon Dec 18 16:56:57 2023
...
def FiniteDifferenceValidation(self, perturbationIndex=0, errorTolerance=5e-2):
    ...
    # initial guess
    x0 = self.lowerBound + np.random.rand(self.ForwardSolve.numberOfNodes) * (
    ...
    error.append(
        100 * (objectiveFiniteDifference - objectiveAdjoint) / objectiveFiniteDifference
    )

    # constraint error
    for i in range(len(c0)):
        constraintFiniteDifference = (c1[i] - c0[i]) / perturbation
        constraintAdjoint = dcdrho0[
            (i * self.ForwardSolve.numberOfNodes) + perturbationIndex
        ]
        error.append(
            100
