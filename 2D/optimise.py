"""
Created on Sat Oct  7 21:56:37 2023
...
class OptimisationLoop:
    ...
    def __init__(self):
        ...
        # output folder
        self.outputFolder = (
            "results/TO" + datetime.datetime.now().strftime("%y_%m-%d-%H-%M-%S") + "/"
        )
    ...
    def OptimisationSetup(self):
        ...
        self.j, self.djdrho, self.c, self.dcdrho = self.ForwardSolve.Solve(
            self.designVariables0
        )
        ...
        if self.scalingInitialisation is True:
            ...
            self.jacobianScaling = np.zeros(
                len(self.c) * self.ForwardSolve.numberOfNodes
            )
